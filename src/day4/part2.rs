...
fn find_square_on_boards(draw: u8, boards: &mut Vec<Board>, counts: &mut Vec<(Vec<u8>, Vec<u8>)>) -> Option<u16> {
    ...
    if num_boards > 1 {
        boards.retain_mut(|board| {
            if find_square_on_board(draw, board, &mut counts[i]) && num_boards > 1 {
                num_boards -= 1;
                counts.remove(i);
                false
            } else {
    ...
    // If there is only 1 board left, this will keep executing until the board wins
    if find_square_on_board(draw, &mut boards[0], &mut counts[0]) && num_boards == 1 {
        Some(sum_unmarked(&boards[0]))
    } else {
        None
    }
}
