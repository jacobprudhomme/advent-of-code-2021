...
fn follow_trajectory(trajectory: Vec<Direction>) -> (isize, isize) {
    ...
    for direction in trajectory {
        match direction {
            Direction::Forward(distance) => position.0 += distance as isize,
            Direction::Down(distance) => position.1 += distance as isize,
            Direction::Up(distance) => position.1 -= distance as isize,
        }
    }
