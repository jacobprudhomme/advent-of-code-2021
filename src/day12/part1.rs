...
fn traverse_map(map: &IndexMap<Cave, Vec<Cave>>, mut visited: IndexSet<Cave>, curr_cave: Cave) -> Vec<Vec<Cave>> {
    match curr_cave {
        Cave::End => vec!(vec!(Cave::End)),
        _ => {
            if let Cave::Small(_) = curr_cave {
                visited.insert(curr_cave.clone());
            }
            ...
            map[&curr_cave]
                .to_owned()
                .into_iter()
                .filter(|neighbour| *neighbour != Cave::Start && !visited.contains(neighbour))
                .fold(Vec::new(), |mut acc, next_cave| {
                    let mut paths = traverse_map(map, visited.clone(), next_cave);
...
fn count_paths(map: IndexMap<Cave, Vec<Cave>>) -> usize {
    let paths = traverse_map(&map, IndexSet::new(), Cave::Start);
    paths.len()
}
